...
export const RichText: React.FC<RichTextProps> = ({ content, className = "" }) => {
 ...
 return <strong key={idx} className="font-bold ui-strong">{part.slice(2, -2)}</strong>;
 }
 if (part.startsWith("*") && part.endsWith("*") && !part.startsWith("**")) {
 return <em key={idx} className="italic ui-muted">{part.slice(1, -1)}</em>;
 }
 if (part.startsWith("`") && part.endsWith("`")) {
 return (
 <code key={idx} className="rounded ui-panel px-1 py-0.5 font-mono text-xs text-accent">
 {part.slice(1, -1)}
 </code>
 );
 }
 ...
 return (
 <span key={idx} className="inline-flex items-center gap-1 rounded tone-accent px-1.5 py-0.5 text-[10px] font-extrabold text-accent shadow-sm border border-token">
 {part.replace(/\*/g, "")}
 </span>
 );
 }
 ...
 return (
 <h3 key={bIdx} className="font-bold text-base ui-strong mt-2 mb-1">
 ...
 if (trimmed.startsWith("## ")) {
 return (
 <h2 key={bIdx} className="font-bold text-lg ui-strong mt-3 mb-1">
 ...
 if (trimmed.startsWith("# ")) {
 return (
 <h1 key={bIdx} className="font-bold text-xl ui-strong mt-3 mb-2">
 ...
 return (
 <li key={lIdx} className="flex items-start gap-2 ui-strong">
 ...
 return (
 <p key={bIdx} className="ui-strong">
 {lines.map((line, lIdx) => (
