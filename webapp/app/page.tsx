...
import { canEditOrDeleteJob, canManageVacancies, DEFAULT_YOUTUBE_PLACEHOLDER, getYouTubeEmbedUrl, normalizeEmail } from "./auth-policy";
...
const emptyDraft: AdminDraft = { title: "", company: "", type: "Permanent", specialization: "", customSpecialization: "", locationMode: "malaysia", state: "", country: "", salary: "", vacancies: 1, minimumRequirement: "Diploma", email: "", youtubeUrl: DEFAULT_YOUTUBE_PLACEHOLDER };
...
export default function Home() {
  ...
  const draftBenchmark = useMemo(() => {
    ...
    return benchmarkFor({ title: draft.title, specialization: effectiveSpec } as Job);
  }, [draft.title, draft.specialization, draft.customSpecialization]);
  ...
  async function saveVacancy(event: FormEvent) {
    ...
    if (!draft.title.trim() || !draft.company.trim() || !effectiveSpecialization || !location) {
  ...
  function editCustomJob(job: Job) {
    ...
    const isPredefined = predefinedSpecs.includes(job.specialization);
    ...
    <main id="top">
      ...
      {adminOpen && canManageJobs && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setAdminOpen(false); }}><section className="admin-panel" role="dialog" aria-modal="true" aria-labelledby="admin-title">
        ...
        <form onSubmit={saveVacancy} className="admin-form"><label>Job title<input required value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })}/></label><label>Company<input required value={draft.company} onChange={e => setDraft({ ...draft, company: e.target.value })}/></label><label>Type<select value={draft.type} onChange={e => setDraft({ ...draft, type: e.target.value })}><option>Permanent</option><option>Internship</option><option>Contract</option><option>Part-time</option></select></label><label>Specialization<select required value={draft.specialization} onChange={e => setDraft({ ...draft, specialization: e.target.value })}><option value="" disabled>Select specialization</option>{specializations.filter(item => item !== "All specializations").map(item => <option key={item} value={item}>{item}</option>)}<option value="Other">Other (Specify below)</option></select></label>
          ...
          <div className="full rounded-xl border border-indigo-200 dark:border-indigo-900 bg-indigo-50/80 dark:bg-indigo-950/40 p-3.5 my-1 text-xs">
            <div className="flex items-center gap-1.5 font-bold text-indigo-900 dark:text-indigo-200">
              <span>📊</span>
              <span>Malaysia Workforce Salary Benchmark Helper (DOSM 2024 Survey)</span>
            </div>
            <p className="mt-1 text-indigo-800 dark:text-indigo-300">
              Suggested benchmark for <b>{draft.title.trim() || "this role"}</b>:{" "}
              <span className="font-extrabold text-indigo-950 dark:text-indigo-100 text-sm">
                RM {draftBenchmark.amount.toLocaleString()} / month
              </span>{" "}
              <i>({draftBenchmark.label})</i>
            </p>
            <small className="text-[10px] text-indigo-600 dark:text-indigo-400 block mt-0.5">
              Use this benchmark to guide realistic compensation offers when creating vacancies.
            </small>
          </div>
