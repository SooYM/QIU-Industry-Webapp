...
### 1.1 [layout.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/layout.tsx)
...
### 1.2 [page.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/page.tsx)
...
### 1.3 [auth-context.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/auth-context.tsx)
...
### 1.4 [auth-policy.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/auth-policy.ts)
...
### 1.5 [firebase-client.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/firebase-client.ts)
...
### 1.6 [globals.css](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/globals.css)
...
### 1.7 [RichText.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/RichText.tsx)
...
### 1.8 [chat.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/chat.ts)
...
### 1.9 [slm-engine.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/slm-engine.ts)
...
### 1.10 [recommendation.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/recommendation.ts)
- **Main Purpose & Architectural Role**: Academic grade-aware job recommendation engine. Evaluates candidate academic transcripts, course grades ($A+$ through $F$), FYP technologies, and degree majors against job requirements to compute match scores, recommendation highlights, and strict grade-based exclusion rules.
- **Exported Symbols**: `JobRecommendationResult`, `evaluateJobForStudent(job, student)`, `rankJobsForStudent(jobs, student)`.
- **Detailed Code Block Breakdown**:
  - `subjectKeywords` (`L12-L30`): Map connecting academic subject areas (`machine-learning`, `cybersecurity`, `web-dev`, `data-analytics`, `accounting`, `finance`, `hospitality`, `pharmacy`, etc.) to target vacancy search keywords.
  - `evaluateJobForStudent` (`L32-L96`): Evaluates a job against a student profile. Initializes base score at 50. Checks course grade matches: high grades ($A+$, $A$, $A-$) grant $+35$ to $+45$ score boosts and set status to `"recommended"`; low/failing grades ($C+$, $D$, $F$) penalize score by $-60$, add exclusion notes, and set status to `"excluded"`. Adds $+15$ boost for FYP title/technology matches. Clamps final score between 0 and 99.
  - `rankJobsForStudent` (`L98-L113`): Maps `evaluateJobForStudent` across a job list and sorts items so recommended jobs appear first and excluded jobs sit at the bottom.

### 1.11 [student-data.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/student-data.ts)
- **Main Purpose & Architectural Role**: Domain type definitions for student profiles, academic course grades, Final Year Projects (FYP), and extracurricular leadership positions. Includes GPA mapping constants and sample student profile mock datasets representing various academic disciplines.
- **Exported Symbols**: `Grade`, `CourseGrade`, `FinalYearProject`, `ExtracurricularPosition`, `StudentProfile`, `gradeGpaMap`, `sampleStudentProfiles`.
- **Detailed Code Block Breakdown**:
  - Type Definitions (`L1-L61`): Interfaces for `CourseGrade` (code, name, grade, gpa, category, subjectArea), `FinalYearProject` (title, abstract, supervisor, technologies, grade), `ExtracurricularPosition` (role, organization, period, description), and `StudentProfile` (id, fullName, email, cgpa, courses, fyp, extracurriculars).
  - `gradeGpaMap` (`L63-L75`): Canonical GPA scale map ($A+/A = 4.0$, $A- = 3.7$, $B+ = 3.3$, $B = 3.0$, $B- = 2.7$, $C+ = 2.3$, $C = 2.0$, $D+ = 1.3$, $D = 1.0$, $F = 0.0$).
  - `sampleStudentProfiles` (`L77-L523`): Comprehensive array of student profiles spanning Computer Science ML stars, Accountancy/ACCA candidates, BBA majors, Hospitality management, Mass Communication, Mechatronics robotics, Biotech food science, TESL educators, Psychology, Pharmacy, and academic failure retry cases.

### 1.12 [student-modal.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/student-modal.tsx)
- **Main Purpose & Architectural Role**: Interactive student profile and transcript inspection modal dialog. Allows reviewing candidate academic grades, recommendation impacts, FYP abstracts, leadership positions, switching demo student profiles, and launching the 1-click CV generator.
- **Exported Symbols**: `StudentModal({ currentStudent, isOpen, onClose, onSelectStudent, onOpenCvGenerator })`.
- **Detailed Code Block Breakdown**:
  - `activeTab` State (`L21`): Controls active sub-tab view (`transcript`, `fyp`, `leadership`, `profiles`).
  - Header & Action Bar (`L38-L70`): Displays student full name, CGPA badge, university, major, close button, and 1-Click Generate CV action button.
  - Tab Navigation (`L71-L117`): Tab bar switching between transcript grade table, FYP details, leadership experience, and profile switcher.
  - Transcript View (`L121-L175`): Explanatory callout banner and structured HTML table listing course code, subject name, category, grade badge, and recommendation impact (high boost vs grade exclusion).
  - FYP & Leadership Views (`L177-L225`): Renders project title, grade, supervisor, technology pills, abstract, and club leadership role cards.
  - Profile Switcher View (`L227-L267`): Card grid displaying demo student profiles with CGPA, top grades, low grades, and selection handlers.

### 1.13 [cv-generator.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/cv-generator.tsx)
- **Main Purpose & Architectural Role**: Standalone CV modal dialog component offering ATS single-column formatting, modern professional dual-column layout previewing, live text editing, plain-text ATS copying to clipboard, and print/PDF generation.
- **Exported Symbols**: `CvGeneratorModal({ student, isOpen, onClose, onUpdateStudent })`.
- **Detailed Code Block Breakdown**:
  - State Controls (`L14-L18`): Controls active format (`ats` vs `professional`), edit mode (`isEditing`), copy feedback (`copied`), and working profile (`editProfile`).
  - `generatePlainTextCv` (`L25-L74`): Formats student profile into clean, plain-text ASCII layout suitable for ATS job portal pasting.
  - `handleCopyText` (`L76-L81`): Copies plain-text CV string to user clipboard and displays confirmation toast.
  - Inline Form Editor (`L172-L229`): Form fields allowing real-time edits to full name, email, university, major, and professional summary.
  - ATS Layout Renderer (`L230-L328`): Single-column serif layout optimized for ATS parsers, featuring clean header, professional summary, education block, coursework grid, FYP details, and leadership items.
  - Professional Layout Renderer (`L329-L463`): Styled modern layout featuring header banner, contact badges, CGPA statistics highlight bar, grade pills, technology tags, and card sections.

### 1.14 [map-tooltip.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/map-tooltip.ts)
...
### 1.15 [chatgpt-auth.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/app/chatgpt-auth.ts)
- **Main Purpose & Architectural Role**: Server-side authentication header helper utility for parsing OpenAI/ChatGPT OAuth proxy headers (`oai-authenticated-user-email`, `oai-authenticated-user-full-name`). Provides user retrieval and route guard helpers for external integration endpoints.
- **Exported Symbols**: `ChatGPTUser`, `getChatGPTUser()`, `requireChatGPTUser(returnTo)`, `chatGPTSignInPath(returnTo)`, `chatGPTSignOutPath(returnTo)`.
- **Detailed Code Block Breakdown**:
  - Header Constants (`L10-L17`): Header key definitions for authenticated email, full name, and UTF-8 encoding type.
  - `getChatGPTUser` (`L19-L36`): Reads request headers, decodes percent-encoded UTF-8 full name if present, and constructs `ChatGPTUser` object.
  - `requireChatGPTUser` (`L38-L45`): Asserts authenticated user presence, triggering Next.js `redirect()` to sign-in path if unauthenticated.
  - `safeRelativeReturnPath` (`L57-L70`): Validates return path strings to prevent open redirect vulnerabilities, ensuring return URLs remain relative to application origin.

---

## 2. Reusable UI Components (`webapp/components/`)

### 2.1 [Modal.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/components/Modal.tsx)
...
### 2.2 [toast.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/components/toast.tsx)
...
### 2.3 [ImagePreview.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/components/ImagePreview.tsx)
...
#### [Guide.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/Guide.tsx)
...
#### 3.2.1 [home/HomeView.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/home/HomeView.tsx)
...
#### 3.2.2 [home/useLogoBackdrop.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/home/useLogoBackdrop.ts)
...
#### 3.3.1 [vacancies/VacancyList.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/vacancies/VacancyList.tsx)
- **Main Purpose & Architectural Role**: Vacancy card grid container and pagination controller. Renders `<VacancyCard>` items, empty state reset prompts, and pagination controls.
- **Exported Symbols**: `VacancyList({ jobs, isStudent, course, appliedIds, columns, currentPage, pageCount, onGlow, onSelect, onPrev, onNext, onReset })`.
- **Detailed Code Block Breakdown**:
  ...
  - Pagination Controls (`L52`): Renders Previous/Next buttons and page count indicators when `pageCount > 1`.

#### 3.3.2 [vacancies/VacancyCard.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/vacancies/VacancyCard.tsx)
...
#### 3.3.3 [vacancies/VacancyModal.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/vacancies/VacancyModal.tsx)
...
#### 3.3.4 [vacancies/VacancyFilters.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/vacancies/VacancyFilters.tsx)
...
#### 3.3.5 [vacancies/vacancy-utils.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/vacancies/vacancy-utils.ts)
...
#### 3.4.1 [admin/AdminPanel.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/AdminPanel.tsx)
...
#### 3.4.2 [admin/AdminSummary.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/AdminSummary.tsx)
...
#### 3.4.3 [admin/ApprovalQueue.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/ApprovalQueue.tsx)
...
#### 3.4.4 [admin/CompanyManager.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/CompanyManager.tsx)
...
#### 3.4.5 [admin/EmployerSummary.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/EmployerSummary.tsx)
...
#### 3.4.6 [admin/StudentActivity.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/StudentActivity.tsx)
...
#### 3.4.7 [admin/ResumeViewer.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/ResumeViewer.tsx)
...
#### 3.4.8 [admin/ChatHistory.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/ChatHistory.tsx)
...
#### 3.4.9 [admin/SettingsPanel.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/admin/SettingsPanel.tsx)
...
#### 3.5.1 [student/StudentHistory.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/student/StudentHistory.tsx)
...
#### 3.5.2 [student/StudentResume.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/student/StudentResume.tsx)
...
#### 3.5.3 [student/GeneratedCV.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/student/GeneratedCV.tsx)
...
#### 3.5.4 [student/cv-download.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/student/cv-download.ts)
...
#### 3.6.1 [events/EventsView.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/EventsView.tsx)
...
#### 3.6.2 [events/EventDetail.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/EventDetail.tsx)
...
#### 3.6.3 [events/EventForm.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/EventForm.tsx)
...
#### 3.6.4 [events/EventPresenter.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/EventPresenter.tsx)
...
#### 3.6.5 [events/SpeakerAvatar.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/SpeakerAvatar.tsx)
...
#### 3.6.6 [events/EventAttendance.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/events/EventAttendance.tsx)
...
### 3.7 Grounded Assistant Subsystem (`features/chat/`)

#### [chat/ChatAssistant.tsx](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/features/chat/ChatAssistant.tsx)
- **Main Purpose & Architectural Role**: Grounded floating AI Job Assistant chat widget. Integrates on-device **SLM-Lite v1.0** RAG engine (`answerFromJobs`), renders markdown responses (`<RichText>`), displays interactive source citation buttons, auto-scrolls chat history, and logs turns to `chat_logs`.
- **Exported Symbols**: `ChatAssistant({ open, jobs, onClose, onSelectSource })`.
- **Detailed Code Block Breakdown**:
  - Scroll Effect (`L27-L33`): Automatically scrolls message container to bottom whenever new messages arrive.
  - `askAssistant` Handler (`L35-L57`): Extracts query input, calls `answerFromJobs(question, jobs)`, appends assistant response with candidate source jobs, and logs turn to Firestore `chat_logs` via `logChat`.
  - Chat Widget UI (`L61-L69`): Floating chat drawer rendering SLM-Lite badge header, scrollable message list with `<RichText>` content and clickable source job pills, input form, and on-device model attribution footer.

---

## 4. Data & Domain Layer (`webapp/lib/`)

### 4.1 [lib/data/types.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/lib/data/types.ts)
...
### 4.2 [lib/data/firestore.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/lib/data/firestore.ts)
- **Main Purpose & Architectural Role**: Unified Cloud Firestore access layer. Serves as the single integration seam between UI components and the database. Enforces strict schema hygiene, atomic counter operations (`increment`), real-time `onSnapshot` listeners, two-step attendance duration calculations, and batch data resets.
- **Exported Symbols**: `COLLECTIONS`, `DEFAULT_SETTINGS`, `isApproved(job)`, `subscribeVacancies`, `saveJob`, `deleteJob`, `stageJobEdit`, `approveJob`, `rejectJob`, `subscribeJobStats`, `recordApplication`, `deleteApplication`, `recordView`, `subscribeApplications`, `subscribeViews`, `saveResume`, `deleteResume`, `subscribeResumes`, `subscribeMyResume`, `logChat`, `subscribeAllChats`, `subscribeCompanyChats`, `subscribeEvents`, `saveEvent`, `deleteEvent`, `setEventCode`, `stopEventCode`, `ccaThresholdMinutes`, `checkInAttendance`, `checkOutAttendance`, `getMyAttendance`, `subscribeAttendance`, `subscribeCompanies`, `saveCompany`, `deleteCompany`, `approveCompany`, `stageCompanyEdit`, `applyCompanyEdit`, `rejectCompanyEdit`, `clearCompanies`, `subscribeSettings`, `saveSettings`, `submitSignup`, `subscribeMySignup`, `subscribeSignups`, `approveSignup`, `deleteSignup`, `resetAllData`.
- **Detailed Code Block Breakdown**:
  - `COLLECTIONS` Constant (`L11-L26`): Maps collection identifiers (`users`, `vacancies`, `whitelisted_emails`, `applications`, `resumes`, `events`, `event_codes`, `attendance`, `job_stats`, `companies`, `app_settings`, `employer_signups`).
  - Vacancy Operations (`L48-L88`): Real-time subscription (`subscribeVacancies`), job save (`saveJob`), deletion (`deleteJob`), staged edit (`stageJobEdit`), approval (`approveJob`), and rejection (`rejectJob`).
  - Atomic Tally & Application Operations (`L92-L136`): Updates public counters in `job_stats/{jobId}` via `increment(delta)`. Records applications (`recordApplication`), deletes applications (`deleteApplication`), and subscribes to student feeds.
  - Resume & Chat Operations (`L139-L177`): Manages student resume documents and logs grounded assistant conversation turns to `chat_logs`.
  - Attendance Engine & Duration Math (`L199-L244`): `ccaThresholdMinutes` computes required physical attendance minutes ($\ge 80\%$ or 45 min floor). `checkInAttendance` creates check-in doc; `checkOutAttendance` computes elapsed duration ($checkOutMs - checkInMs$), evaluates `caEligible` status, and updates document.
  - Exhibitor Showcase Operations (`L247-L296`): Subscribes to `companies` collection, handles exhibitor creation, staged edit diff approvals (`applyCompanyEdit`), and bulk clear (`clearCompanies`).
  - Settings & Signups (`L298-L372`): Persists portal settings (`saveSettings`), handles employer self-registration requests (`submitSignup`), and processes administrative approvals (`approveSignup`).
  - Master Database Reset (`L378-L393`): `resetAllData` iterates through all Firestore collections, deleting documents while preserving superadmin credentials.

### 4.3 [lib/data/course-map.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/lib/data/course-map.ts)
...
### 4.4 [lib/auth/course-directory.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/lib/auth/course-directory.ts)
...
### 4.5 [lib/theme/tokens.css](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/lib/theme/tokens.css)
...
### 5.1 [firestore.rules](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/firestore.rules)
...
### 5.2 [storage.rules](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/storage.rules)
...
### 5.3 [scripts/generate_data.py](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/scripts/generate_data.py)
...
### 5.4 [package.json](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/package.json)
...
### 5.5 [next.config.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/next.config.ts)
...
### 5.6 [vite.config.ts](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/vite.config.ts)
- **Main Purpose & Architectural Role**: Vite development server and Cloudflare build configuration plugin file.
- **Exported Symbols**: `default defineConfig()`.
- **Detailed Code Block Breakdown**:
  - Server & Plugin Setup (`L36-L59`): Configures HMR file watcher polling for macOS Seatbelt sandboxes, integrates `vinext()` plugin, `sites()` plugin, and `@cloudflare/vite-plugin` with local D1 and R2 binding configurations.

### 5.7 [postcss.config.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/postcss.config.mjs)
...
### 5.8 [eslint.config.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/eslint.config.mjs)
...
### 6.1 [tests/academic-recommendation-cv.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/academic-recommendation-cv.test.mjs)
- **Main Purpose & Architectural Role**: Unit test suite for course grade-aware recommendation engine. Validates grade boosts ($A+$ in Machine Learning) and grade exclusions ($D+$ in Cybersecurity).
- **Exported Symbols**: Executable Node.js test suite (`npm test`).
- **Detailed Code Block Breakdown**:
  - Tests (`L6-L62`): Verifies high grade match score boost ($\ge 80$), asserts exclusion status for poor grades, verifies recommendation prioritization for network security students, and tests sorting order (`rankJobsForStudent`).

### 6.2 [tests/admin-form-regression.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/admin-form-regression.test.mjs)
...
### 6.3 [tests/chat-retrieval.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/chat-retrieval.test.mjs)
...
### 6.4 [tests/employer-whitelist.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/employer-whitelist.test.mjs)
...
### 6.5 [tests/firestore-rules.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/firestore-rules.test.mjs)
...
### 6.6 [tests/map-tooltip-regression.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/map-tooltip-regression.test.mjs)
...
### 6.7 [tests/slm-engine.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/slm-engine.test.mjs)
...
### 6.8 [tests/student-recommendation.test.mjs](file:///Users/sooyauming/Desktop/Intern/Vacancy%20Portal/webapp/tests/student-recommendation.test.mjs)
- **Main Purpose & Architectural Role**: Unit test suite for student profile data structures and course matching algorithms.
- **Exported Symbols**: Executable Node.js test suite (`npm test`).
- **Detailed Code Block Breakdown**:
  - Tests (`L12-L93`): Verifies integrity of sample student profiles, tests grade-aware recommendation boosts and exclusions, checks ranking order, tests automatic computer science major matching, verifies multi-faculty representation, and asserts strict exclusion for failed course grades ($F$).

---

## 7. Summary Verification & Quality Gates

All files documented above correspond directly to active project source and configuration files. To verify system integrity and quality gates before deployment:

```bash
# Execute unit and regression test suite
npm test

# Execute emulator-backed Firestore Security Rules test suite
npm run test:rules

# Run ESLint syntax and typing verification
npm run lint
```
