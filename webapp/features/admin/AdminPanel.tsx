...
export function AdminPanel({
  ...
  const cancelVacancyEdit = () => { setEditingId(null); setDraft(emptyDraft); setTitleCommitted(false); setAdminMessage(""); };
  // Advisory salary range from the HR reference sheet, matched to the typed title.
  // Employers see it as guidance while posting; admins already know their ranges.
  const salaryHint = isEmployer ? salaryBandFor(draft.title) : null;
  ...
  const adminFilteredJobs = useMemo(() => {
    ...
    const filtered = customJobs.filter((job) =>
      // Employers manage only their own vacancies; admins see every listing.
      (!isEmployer || canEditOrDeleteJob(job, user?.email, role)) &&
    ...
    switch (adminSort) {
      case "oldest": return filtered.sort((a, b) => a.id - b.id);
      case "title_az": return filtered.sort((a, b) => a.title.localeCompare(b.title));
      case "title_za": return filtered.sort((a, b) => b.title.localeCompare(a.title));
      case "company_az": return filtered.sort((a, b) => a.company.localeCompare(b.company));
      case "company_za": return filtered.sort((a, b) => b.company.localeCompare(a.company));
      default: return filtered.sort((a, b) => b.id - a.id); // newest first
    }
  }, [customJobs, adminQuery, adminCompany, adminSpecialization, adminType, adminSort, isEmployer, user, role]);
  ...
  async function saveVacancy(event: FormEvent) {
    ...
    // Tolerate thousands separators / spaces ("180,000") — the only rule is > 0,
    // with no upper cap.
    const salary = Number(draft.salary.replace(/[,\s]/g, ""));
    if (!(salary > 0)) {
      setAdminMessage("Enter the monthly salary in RM (any amount above 0).");
      return;
    }
  ...
  function editCustomJob(job: Job) {
    ...
    // Setting editingId opens the edit form in a modal popup (see render), so no
    // scrolling is needed — the editor is always centred in view.
  }
