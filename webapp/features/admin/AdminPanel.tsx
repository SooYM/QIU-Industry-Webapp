...
export function AdminPanel({
  ...
  // Scroll the edit form into view once it has actually rendered. In the "Manage
  // vacancies" tab the form only mounts when editingId flips non-null, so scrolling
  // inside the click handler (before the commit) would find no element.
  useEffect(() => {
  ...
  function editCustomJob(job: Job) {
    ...
    // The scroll is handled by an effect once the form has committed (see above).
  }
  ...
  return (
    <section className="admin-panel admin-inline" aria-labelledby="admin-title">
      ...
      {(adminView === "manage" || adminView === "addVac" || editingId !== null) && <form ref={formRef} onSubmit={saveVacancy} className="admin-form"><label>Job title<input required value={draft.title} onChange={e => { setDraft({ ...draft, title: e.target.value }); setTitleCommitted(false); }} onBlur={() => setTitleCommitted(true)}/></label>{isApprover
