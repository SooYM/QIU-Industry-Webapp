...
export function EventPresenter({ event, rotateSeconds, ccaSettings, onClose }: {
  ...
  ccaSettings?: { ccaPercent: number; ccaFloorMinutes: number };
  onClose: () => void;
}) {
  const refreshMs = Math.max(5, rotateSeconds) * 1000;
  const [step, setStep] = useState<"checkin" | "checkout">("checkin");
  const [qr, setQr] = useState("");
  const [count, setCount] = useState(refreshMs / 1000);

  useEffect(() => {
    let active = true;
    async function rotate() {
      const code = randomCode();
      const codeExpiry = Date.now() + refreshMs + 6000; // small grace for scan latency
      try {
        ...
        if (active) { setQr(dataUrl); setCount(refreshMs / 1000); }
  ...
  return (
    <Modal className="admin-panel event-presenter" labelledBy="presenter-title" closeLabel="Stop presenting" onClose={close}>
      ...
      <p className="text-accent text-xs">Keep this on the projector. CCA credit requires ≥ {ccaThresholdMinutes(event.sessionMinutes, ccaSettings)} min between check-in and check-out.</p>
    </Modal>
  );
}
